...
function prepareMaintenanceDataForExport(records: MaintenanceRecord[]) {
  return records.map(record => {
    ...
    return {
      ...
      'Service Start Date': record.service_start_date ? format(new Date(record.service_start_date), 'dd-MM-yyyy') : '',
      'Service End Date': record.service_end_date ? format(new Date(record.service_end_date), 'dd-MM-yyyy') : '',
      'Invoice Number': record.invoice_number,
      'Invoice Date': record.service_start_date ? format(new Date(record.invoice_date), 'dd-MM-yyyy') : '',
